...
pub const System = struct {
  world: *ecs.World,
  start_rotations: std.AutoHashMap(ecs.EntityID, Module.Components.Rotation.Component),
  ...
  pub fn init(world: *ecs.World) System {
    ...
    self.start_rotations = std.AutoHashMap(ecs.EntityID, Module.Components.Rotation.Component).init(world.allocator);
    return self;
  }
  ...
  pub fn update(self: *System) void {
    ...
    while(it.next()) |entry| {
      ...
      var start: Module.Components.Rotation.Component = undefined;
...
test "System should update rotation" {
  ...
  if (entity.world.components.rotation.get(entity.id)) |rotation|
    try tst.expectEqual(Module.Components.Rotation.Component{.x = 0.5, .y = -0.5, .z = 50}, rotation)
   else
